...
| 35 | `/quality/gates` — the index of the checks, generated from the scripts rather than typed: 195 named checks parsed out of their `ok(…)` calls with the condition each asserts, the commit that last wrote each line (a `git blame` index, refreshed by `npm run gates:index` and gated for freshness), the 12 markup rules with the failure each prevents, and the 7 checks no command runs | `0730278` | `check:exports`: every name printed, the derived counts, the run-time-composed names admitted, the manual checks named, the index fresh |
...
**Open, and worth saying so:** items 21–30, 36–37, 40–49, 51–60, 62, 64–70,
